...
export async function POST(request: Request) {
  try {
    ...
    const duplicate = await sql`select id from items where wishlist_id = ${wishlist.id} and original_url = ${body.originalUrl} limit 1`;
    const rows = await sql`
      insert into items (wishlist_id, name, description, image_url, original_url, domain)
      values (${wishlist.id}, ${body.name}, ${body.description || null}, ${body.imageUrl || null}, ${body.originalUrl}, ${domainFromUrl(body.originalUrl)})
      returning *
    `;
